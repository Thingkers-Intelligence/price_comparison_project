...
class ShwapnoFishMeatSpider(scrapy.Spider):
    name = 'shwapno_fish_meat'
    ...
    def start_requests(self):
        yield scrapy.Request(
            'https://www.shwapno.com/fish-meat',
    ...
    def parse(self, response):
        ...
        for product in products:
            ...
            quantity_text = (
                product.css('button.product-box-quantity::text').get() or
                product.css('select.product-box-quantity option::text').get() or
                product.css('.product-price span:last-child::text').get()
            )
